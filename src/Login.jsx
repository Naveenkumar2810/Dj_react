...
const Login = () => {
  ...
  const [form,setform]= useState({
    type:true,  //If form type is true login oage else sign up page
    error_msg:null
  })
  ...
  const set_cred = (e)=>{  //Function to assign values and colours to the form fields and sending in requests 
    ...
    //  console.log(name)
     ...
     switch(name) {
      ...
      case "username":
         console.log('Going to change username color')
      ...
      case "repeat-password":
        setCred({...cred,rep_password_color:value===cred.password?"text-green-400":"text-red-600"})
        break
      default:
        return null
    }
  ...
  const Sign_up = async (e) => {  //function handle creating new user 
      ...
      try {
        const response = await axios.post('http://127.0.0.1:8000/Api/Signup/', 
            ...
            response.status===200? console.log('user created'):console.log('Wrong credentials provided')
          }
        catch (error){
          console.log(error)
        };
    ...
    <div className="border border-black w-full h-full flex justify-center items-center rounded-xl bg-[url('https://naveen28.s3.eu-north-1.amazonaws.com/signup_page.jpeg')] bg-cover bg-center p-6">
      <form onSubmit={!form.type?(e)=>{Sign_up(e)}:(e)=>{authentication(e)}}  className="w-full md:w-2/5 h-auto mx-auto min-h-3/5  p-5 rounded-2xl bg-gray-900/70 backdrop-blur-signup overflow-hidden">
        ...
        {!form.type?<div className="md:mb-5 md:mt-10 mb-2 mt-5 border-[1px] border-[rgba(255,255,255,0.25)] w-full md:w-5/6 mx-auto flex flex-row rounded-full">  
          <input onChange={(e)=>{set_cred(e)}} name='username'  type="text" id="username" className="w-[calc(80%)] shadow-sm text-white text-md block p-5 bg-transparent appearance-none focus:outline-none" placeholder='Enter user name'required/>
        ...
        <div className="md:mb-5 md:mt-10 mb-2 mt-5 border-[1px] border-[rgba(255,255,255,0.25)] w-full md:w-5/6 mx-auto flex flex-row rounded-full">  
          <input onChange={(e)=>{set_cred(e)}} name='mobile_number'  type="text" id="mobile_number" className="w-[calc(80%)] shadow-sm text-white text-md block p-5 bg-transparent appearance-none focus:outline-none" placeholder='Enter mobile number'required/>
        ...
        <div className="md:mb-5 md:mt-10 mb-2 mt-5 border-[1px] border-[rgba(255,255,255,0.25)] w-full md:w-5/6 mx-auto flex flex-row rounded-full">  
          <input onChange={(e)=>{set_cred(e)}} name='password'  type="password" id="password" className="w-[calc(80%)] shadow-sm text-white text-md block p-5 bg-transparent appearance-none focus:outline-none" placeholder='Enter password'required/>
        ...
        {!form.type? <div className="md:mb-5 md:mt-10 mb-2 mt-5 border-[1px] border-[rgba(255,255,255,0.25)] w-full md:w-5/6 mx-auto flex flex-row rounded-full">  
          <input onChange={(e)=>{set_cred(e)}} name='repeat-password'  type="password" id="rep-password" className="w-[calc(80%)] shadow-sm text-white text-md block p-5 bg-transparent appearance-none focus:outline-none" placeholder='Repeat password'required/>
        ...
        {!form.error_msg && <button type="submit" className="mb-3 md:mt-0 w-full md:w-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none  font-medium rounded-lg text-sm px-5 py-2.5 text-center ">{form.type?"Login":"Create Account"}</button>}
        {!form.error_msg && <span className='ml-5 text-white'>{form.type?"New user ?":"Already a user ?"} <span onClick={()=>{page_switch()}} className='hover:cursor-pointer text-blue-500 hover:text-blue-800'>{form.type?"Sign Up":"Login"}</span></span> }
        {form.error_msg && <div className='text-red-500 font-semibold p-2 rounded-xl flex flex-row justify-center'>
          <span className=''>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-8">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
            </svg>
          </span>
          <span className='p-1'>{form.error_msg}</span>
        </div>}
      </form>
    </div>
  )
