...
import { delete_cartlist } from './App';
...
const Cart = () => {
    ...
    const cart_cost = cart_list.length >0 ? cart_list.reduce((tot,ele)=> tot + Number(ele.Price),0) :0
    console.log('cart total',cart_cost)

    const dispatch=useDispatch()

    const clear_cart__list = ((id)=>{
       dispatch(delete_cartlist(id))
    })


  return (
    <div className='cart-details w-full h-full flex flex-row flex-wrap gap-2 md:gap-3 over md:p-4'>
        <div className='Add-ons h-1/5 md:h-[calc(97%)] md:w-[calc(24%)] w-full shadow-card-hl md:p-4 p-4 flex flex-col md:gap-5 rounded-3xl border-[1px] border-[#cbd5e1]'>
          ...
          <div className='Cool-drinks flex md:flex-col flex-row gap-2 md:h-1/2 h-full'>
             ...
             <div className='Drinks-list w-full h-full md:overflow-y-scroll overflow-x-scroll flex md:flex-col flex-row gap-3 md:p-2 p-1 overflow-hidden md:shadow-card-hl rounded-xl md:border-[3px] border-[#cbd5e1]'>
                <div className='min-w-72 md:min-w-full md:min-h-24 flex flex-row bg-card rounded-2xl border-[1px] border-[#cbd5e1] flex-wrap p-2'>
        ...
        <div className='Cart-value md:h-[calc(97%)] h-4/5 md:w-3/4 w-full flex flex-col md:grid grid-rows-6 grid-cols-5 gap-3'>
          <div className='Cart-value w-full min-h-full row-span-6 col-span-3 shadow-card-hl rounded-xl'>
             ...
             <div className='md:hidden buttons flex flex-row gap-3 p-2'>
                <button onClick = {()=>setPage(true)} className='bg-blue-500 w-1/4 max-w-20 h-1/3 rounded-lg text-sm'>Cart</button>
                <button onClick = {()=>setPage(false)} className='bg-blue-500 w-1/4 max-w-20 h-1/3 rounded-lg text-sm'>Offers</button>
             </div>
             {page?
             <div className='Cart-list w-full min-h-72 max-h-72 p-3 flex flex-col gap-4 md:h-[calc(70%)] md:max-h-[calc(70%)] overflow-y-scroll  border-red-700'>
                 {cart_list? cart_list.map((ele)=>{
                 return (
                 <div className='cart1 w-full md:h-24 h-16 flex flex-row shadow-card-hl rounded-xl p-2 bg-card border-[1px] border-[#cbd5e1]'>
                        ...
                        <div className='desc w-2/4 h-full'>
                           ...
                           <span className='h-1/3 block w-auto text-xs md:text-base text-center'>{ele.Hotel_name}</span>
                        </div>
                        <div className='Quantity w-1/4 h-full place-content-center flex flex-col flex-wrap'>
                          <div className='w-3/5 place-content-center flex flex-row flex-wrap justify-center items-center'>
                             ...
                             <span className='Count w-auto block rounded-full text-sm md:font-semibold px-3 bg-card shadow-card-hl p-1 '>1</span>
                             {/* <button className='md:w-4 w-2 md:h-3 h-1 sub  rounded-full px-1 flex justify-center items-center'>-</button> */}
                             <h1 className='Tot-cost w-full md:text-base text-sm md:mt-4 mt-2'>{`₹${ele.Price}`}</h1>
                          </div>
                          <span onClick={()=>{clear_cart__list(ele.id )}}className='menu-bar w-2/5 h-8 flex items-center md:py-4 md:px-3 p-1 rounded-full mr-1 hover:cursor-pointer'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-7">
                              <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                            </svg>
                          </span>
                        </div>
                 </div>)}):<h1 className='text-xl text-black'>Nothing in list</h1>}
             </div>:
             ...
             <div className='Amt-details w-full h-2/6 flex flex-row flex-wrap px-4 py-2 justify-evenly md:h-[calc(24%)] text-sm md:text-base bg-card'>
                <h1 className='cart-val w-1/2 text-xs md:text-sm text-left'>Total Cart Value</h1>
                <span className='Amt w-1/2 text-xs md:text-sm text-right md:h-1/4'>₹{cart_cost}</span>
                <h1 className='cart-val w-1/2 text-xs md:text-sm text-left md:h-1/4'>Delivery & Handling charges</h1>
                <span className='Amt w-1/2 text-xs md:text-sm text-right md:h-1/4'>₹{0.2 * cart_cost}</span>
                <h1 className='cart-val w-1/2 text-xs md:text-sm text-left text-bold md:h-1/4'>Total</h1>
                <span className='Amt w-1/2 text-right font-bold md:h-1/4'>₹{cart_cost + 0.2 * cart_cost}</span>
